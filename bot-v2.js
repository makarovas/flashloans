...
async function asyncsVar() {
  ...
  uPair0 = new web3.eth.Contract(
    IPair.abi,
    await uFactory.methods.getPair(addrEth, addrDai).call()
  );
  //token pairs
  ...
  sPair = new web3.eth.Contract(
    IPair.abi,
    await sFactory.methods
      .getPair(token0.options.address, token1.options.address)
      .call()
  );
...
newBlockEvent.on("data", async function (blockHeader) {
  try {
    ...
    if (aToB) {
      ...
      const amountOut = await uRouter.methods
        .getAmountOut(amountIn, uReserve0, uReserve1)
        .call();
      ...
      const sAmountIn = await sRouter.methods
        .getAmountIn(amountIn, sReserve1, sReserve0)
        .call();
      ...
      if (difference <= 0) {
        console.log(
          `No arbitrage opportunity on block ${blockHeader.number}\n`
        );
        return;
      }
      ...
      console.log(
        `Block ${blockHeader.number}`.bgBlue +
          ...
          `On Uniswap\n` +
          `${token0Symbol}: ${Math.round(
            uReserve0 / 10 ** 18
          )} | ${token1Symbol}: ${Math.round(uReserve1 / 10 ** 18)}\n\n` +
          `On Sushiswap\n` +
          `${token0Symbol}: ${Math.round(
            sReserve0 / 10 ** 18
          )} | ${token1Symbol}: ${Math.round(sReserve1 / 10 ** 18)}\n\n` +
          `Swap's direction\n` +
          `${token0Symbol} -> ${token1Symbol}\n\n` +
          `Uniswap's pool state\n` +
          `${token1Symbol} excess/${token0Symbol} shortage\n\n` +
          `On Uniswap\n` +
          `Mid price before swap: ${(uReserve0 / uReserve1).toFixed(
            2
          )} ${token0Symbol}/${token1Symbol}\n` +
          `Mid price after swap: ${(newUReserve0 / newUReserve1).toFixed(
            2
          )} ${token0Symbol}/${token1Symbol}\n` +
          `Swap ${Math.round(
            amountIn / 10 ** 18
          )} ${token0Symbol} for ${Math.round(
            amountOut / 10 ** 18
          )} ${token1Symbol}\n` +
          `Trade price: ${(1 / (amountOut / amountIn)).toFixed(
            2
          )} ${token0Symbol}/${token1Symbol} (buy price)\n\n` +
          `Sushiswap price: ${sPrice.toFixed(
            2
          )} ${token0Symbol}/${token1Symbol} (sell price)\n` +
          `Difference: ${difference.toFixed(
            2
          )} ${token1Symbol}/${token0Symbol}\n` +
          `Total difference: ${(totalDifference * priceToken1Eth).toFixed(
            5
          )} ETH or ${totalDifference.toFixed(2)} ${token1Symbol}\n\n` +
          `Gas needed: ${gasNeeded / 10 ** 6}\n` +
          `Gas price: ${gasPrice / 10 ** 9} gwei\n` +
          `Gas cost: ${gasCost.toFixed(5)} ETH\n\n` +
          `${
            profit > 0
              ? `Profit: ${profit.toFixed(5)} ETH or ${(
                  profit * priceEth
                ).toFixed(2)} DAI\n`.green
      ...
      console.log(
        `Block ${blockHeader.number}`.bgBlue +
          ...
          `Mid price after swap: ${(newUReverve0 / newUReverve1).toFixed(
            2
          )} ${token0Symbol}/${token1Symbol}\n` +
          `Swap ${Math.round(
            amountIn / 10 ** 18
          )} ${token1Symbol} for ${Math.round(
            amountOut / 10 ** 18
          )} ${token0Symbol}\n` +
          `Trade price: ${(amountOut / amountIn).toFixed(
            2
          )} ${token0Symbol}/${token1Symbol} (sell price)\n\n` +
          `Sushiswap price: ${sPrice.toFixed(
            2
          ...
          `Total difference: ${(totalDifference * priceToken0Eth).toFixed(
            5
          )} ETH or ${totalDifference.toFixed(2)} ${token0Symbol}\n\n` +
